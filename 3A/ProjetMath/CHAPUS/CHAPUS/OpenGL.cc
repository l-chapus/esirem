...
void droite_parallele(std::vector<float> x ,std::vector<float> y,float& x_M,float& y_M)
{
  ...
  if(x0.at(0)==d1)                                            //si la plus petite distance est [AD]
  {
    x_M=(x.at(0)+x.at(3))/2;                                  //alors on prend le milieu de [AD]
    y_M=(y.at(0)+y.at(3))/2;
  }
  if(x0.at(0)==d2)                                            //si la plus petite distance est [AC]
  {
    x_M=(x.at(0)+x.at(2))/2;                                  //alors on prend le milieu de [AC]
    y_M=(y.at(0)+y.at(2))/2;
  }
  if(x0.at(0)==d3)                                            //si la plus petite distance est [BC]
  {
    x_M=(x.at(1)+x.at(2))/2;                                  //alors on prend le milieu de [BC]
    y_M=(y.at(1)+y.at(2))/2;
  }
  if(x0.at(0)==d4)                                            //si la plus petite distance est [BD]
  {
    x_M=(x.at(1)+x.at(3))/2;                                  //alors on prend le milieu de [BD]
    y_M=(y.at(1)+y.at(3))/2;
  }
}
...
{ 
  ...
  glNewList(4,GL_COMPILE_AND_EXECUTE);  //liste numero 4, cas droites parallèles
  ...
  glNewList(5,GL_COMPILE_AND_EXECUTE);  //liste numero 5, cas droites sécantes
